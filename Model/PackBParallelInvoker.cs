using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

namespace DGEMMSharp.Model;

public partial class DGEMM
{
    internal readonly struct PackBParallelInvoker(int kc, int nc, int nr, ReadOnlyMemory<double> bMem, int ldb, Memory<double> bTo) : IAction
    {
        readonly int kc = kc;
        readonly int nc = nc;
        readonly int nr = nr;
        readonly ReadOnlyMemory<double> bMem = bMem;
        readonly int ldb = ldb;
        readonly Memory<double> bTo = bTo;

        public void Invoke(int jIndex)
        {
            int j = jIndex * nr;
            var bSpan = bMem.Span;
            var bToSpan = bTo.Span;
            ref double bRef = ref Unsafe.AsRef(in bSpan[j]);
            ref double bToRef = ref bToSpan[kc * j];
            int jc = Math.Min(nc - j, nr);
            if (jc != nr)
            {
                int last = jc;
                for (int qc = 0; qc < kc; qc++)
                {
                    Span<double> span = MemoryMarshal.CreateSpan(
                        ref Unsafe.Add(ref bToRef, qc * nr), nr);
                    MemoryMarshal.CreateReadOnlySpan(ref bRef, last)
                        .CopyTo(span);
                    span[last..].Clear();
                    bRef = ref Unsafe.Add(ref bRef, ldb);
                }
            }
            else
            {
                for (int qc = 0; qc < kc; qc++)
                {
                    // Can little span copying be automatically accelerated by SIMD?
                    MemoryMarshal.CreateReadOnlySpan(ref bRef, nr)
                        .CopyTo(MemoryMarshal.CreateSpan(ref bToRef, nr));
                    bRef = ref Unsafe.Add(ref bRef, ldb);
                    bToRef = ref Unsafe.Add(ref bToRef, nr);
                }
            }
        }
    }
}
